...
def stock_deliv_data():
    ...
    data = data[
        ["OPEN_PRICE","HIGH_PRICE","LOW_PRICE","CLOSE_PRICE", "TTL_TRD_QNTY","DELIV_QTY","DELIV_PER"]
    ]
    ...
    fig , ax = mpf.plot(data, addplot=deliv_data, type='candle',style='yahoo',volume=True,
                        returnfig=True, title=f'{symbol} Delivery Percentage',
                        figsize=(20,9), figscale=1)
    st.write(fig)
...
def future_builtup():
    ...
    if to_date < from_date or to_date > datetime.date.today():
        ...
        fig = px.scatter(builtup,"pch_close","pch_oi",hover_data=["SYMBOL"],width=1080,height=420,)
        fig.add_hline(y=0.0)
        fig.add_vline(x=0.0)
...
analysis_dict = {
    ...
    "High/Low Deliv":high_low_deliv,
    "Stock OI Data": stock_oi_data,
    "Future Build-Up":future_builtup,
